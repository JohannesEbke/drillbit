...
class EmptyDitPack(DirectoryDitPack):
    def __init__(self, columns, tablets):
        self.columns, self.tablets = sorted(columns), sorted(tablets)
...
class DitPackSet(object):
    ...
    def columnset_for_packset(self):
        columnset_for_packset = defaultdict(set)
        for column in self.columns:
            packs = frozenset(self.packset_for_column[column])
            columnset_for_packset[packs].add(column)
        return columnset_for_packset
    ...
    def add_missing(self):
        for columnset, tablets in self.missing_tablets_by_columnset():
            print "{0} tablets missing for {1} columns".format(len(tablets), len(columnset))
    ...
    def readers_by_tablet(self):
        missing = self.missing_tablets_by_columnset()
        if missing:
            self.add_missing()
            #for columnset, tablets in missing:
                #print "Missing tablet ids {0} for {1}".format(tablets, sorted(columnset))
            #raise Exception()
            ...
            for columnset, pack_stack in ordered_packs_by_columnset.iteritems():
                while True:
                    ...
                    if read_tablet == tablet:
                        readers[columnset] = pack.next_reader(columnset)
                        break
